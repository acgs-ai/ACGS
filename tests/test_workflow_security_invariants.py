...
@pytest.fixture
def claude_workflows() -> list[pathlib.Path]:
    return sorted((REPO_ROOT / ".github" / "workflows").glob("claude*.yml"))


def test_claude_workflows_gate_trusted_author_associations(
    claude_workflows: list[pathlib.Path],
) -> None:
...
def test_claude_workflows_do_not_request_id_token_write(
    claude_workflows: list[pathlib.Path],
) -> None:
    for workflow in claude_workflows:
