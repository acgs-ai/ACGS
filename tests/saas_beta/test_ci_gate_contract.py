...
def test_saas_beta_required_gate_covers_every_master_pr_without_path_filters() -> None:
    ...
    # Required checks run on Blacksmith Ubuntu 24.04 runners: GitHub-hosted
    # ubuntu-24.04 queues stalled for hours, blocking branch protection.
...
def test_saas_beta_required_gate_has_no_privileged_or_external_side_effect_surface() -> None:
    ...
    # The gate must reference NO secret at all. This is the original blanket
    # ban, restored: it had been narrowed to a single reviewed carve-out for
    # SUBMODULE_TOKEN, the read-only PAT that initialized the private
    # acgs-control-plane submodule. That package is now vendored in-tree, the
    # init step is gone, and the gate needs no credential — so the carve-out
    # is withdrawn rather than left standing unused. The second assertion
    # catches any `secrets.` usage not written as `${{ secrets.NAME }}`.
