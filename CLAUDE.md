...
| `packages/ai-governance-research/` | AI-governance hub — evidence-backed playbooks, controls, and validation (`make validate`) | Markdown + Python |
| `packages/research-engine/` | `delve` — self-deepening research engine | Python 3.11+ |
...
2. **Nested git repos are real.** `packages/acgs-lite`, `packages/Acgs-Swarm`, `packages/clinicalguard`, and `packages/ACGS-agency-agents` (may be absent locally) are independent repos registered in `.gitmodules`. Run `git add` / `git commit` from inside the package, not the parent. Treat parent pointer drift as out-of-scope unless that *is* the task.
...
5. **Two PLAN files exist.** `acgi-ai/PLAN.md` is scoped to the `acgi-ai/` frontend only (there is no root `PLAN.md`). `docs/PLAN-MONOREPO.md` covers monorepo unification. Do not conflate them.
