...
| Wiring | All new checks run inside gates already wired into `pnpm test:all` (`test:csp` carries the posthog-import guard, `test:unit` carries the emitter + route-coverage tests), and `check-ci-readiness-gates.mjs` asserts `test:all` is the CI readiness step — per impl review, that script is CI plumbing, not a per-check registry, and does not enumerate individual `test:*` gates. |
