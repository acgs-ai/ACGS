...
7. Add standardized scripts (per A15): `pnpm test`, `pnpm test:e2e`, `pnpm test:a11y`, `pnpm test:csp`, `pnpm test:contract`, `pnpm test:visual`, `pnpm test:all`. Wire into both workflows. Local static foundation: `pnpm run test:test-surface`; it verifies package, docs, and manifest wiring for `pnpm run test:e2e` and `pnpm run test:visual` (these remain static manifest gates, not Playwright runs). Real browser execution has begun separately via the `browser-checks` CI job (`pnpm test:playwright`): a console-surface axe smoke (blocks serious/critical; `color-contrast` scoped out as documented `--muted` debt) and one console-overview 1440 visual baseline (non-blocking for now) now execute. The full viewport matrix and remaining visual targets remain Phase 3 work.
...
6. **Performance pass.** Lighthouse Performance ≥ 90 on marketing, ≥ 85 on console. Bundle size budget: marketing ≤ 225KB gzipped, console ≤ 350KB gzipped. Wire `vite-bundle-visualizer` to the build for ongoing visibility. Performance budget enforced in CI.
