...
`pnpm test:e2e-http` runs `scripts/smoke-e2e-http-shells.mjs`, starts the mock Vite dev server with `VITE_USE_MOCKS=true` and `VITE_BYPASS_SESSION=true`, and fetches the marketing landing, product slugs including `/products/gove-zone`, login handoff URL, and every in-scope console sidebar route to confirm each path returns the Vite root shell instead of an HTTP/server-error shell. This is local runtime evidence for route availability only; browser Playwright execution remains Phase 2 work before accessibility or interaction claims are upgraded. `pnpm run evidence:browser-workbench` is narrower local Chrome screenshot evidence: it launches marketing and console surfaces separately, uses `VITE_BYPASS_SESSION=true` only in non-production, verifies expected workbench DOM text, scrolls hash targets into view, and rejects likely blank screenshots before writing the local manifest.
