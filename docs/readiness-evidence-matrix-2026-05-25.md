...
| Python typecheck | Local | Root `typecheck-py` runs configured mypy scopes and explicitly skips unconfigured packages | `Makefile`; `tests/test_root_typecheck_gate.py` | Medium | Full strict typing across every legacy package | Separate migration of skipped packages into configured strict mypy scope |
