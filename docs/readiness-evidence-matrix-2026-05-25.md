...
| Python typecheck | Local | Root `typecheck-py` runs configured mypy scopes, now including `acgs-cft-governance-pack`, and explicitly skips remaining unconfigured packages | `Makefile`; `tests/test_root_typecheck_gate.py`; `acgs-cft-governance-pack/pyproject.toml` | Medium | Full strict typing across every legacy package | Separate migration of `packages/Acgs-Swarm` and `acgs_governance_eval_mvp` into configured strict mypy scope |
