...
| Verification is signed by default. | not claimed | `executor.py`, `contracts.py` (`require_signature` defaults `False`) | `test_receipt_signing.py` | Unsigned dev mode is the default; only the local SHA-256 hash is checked, recomputable under host compromise. | "Signing is opt-in; verification is unsigned by default — set `require_signature=True` with a trusted verifier for production." |
| Verification prevents replay (nonce / one-time-use). | not claimed | none | none | Verification is stateless: a valid `ALLOW` receipt is reusable until `expires_at`. Mitigate with short expiry, workflow ledger, side-store. | "No anti-replay nonce; a valid receipt is reusable until it expires (see SECURITY_MODEL.md hardening)." |
