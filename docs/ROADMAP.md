...
| Skill trust: identity, permission ceilings, host interception | ⬜ PLANNED | Bring agent skills (`.claude/skills/**`, `.agents/skills/**`) under governance before their declared permissions can be enforced (steps 5-6 of [`design/agent-skill-trust-pipeline-adoption.md`](design/agent-skill-trust-pipeline-adoption.md)). | Trusted per-skill identity (name/version/artifact digest); independently reviewed permission ceilings held outside the skill; host interception routing governed skill capabilities through `execute_with_receipt`, with declared `permissions:` enforced deny-only against the ceiling; negative-path tests through the real host handler (not only direct policy/executor unit calls) proving the side effect did not run for a forged or mismatched skill digest, an above-ceiling action, and a request missing its authenticated origin. | Skill-driven agent actions become receipt-gated instead of trusted by convention. | Skills remain the least governed executable surface in a governance repository, and skill `permissions:` stay documentation-only. |
