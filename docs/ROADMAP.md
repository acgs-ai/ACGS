...
Status legend: **✅ SHIPPED** — every acceptance sub-clause has a source file *and* a cited test; **🟡 PARTIAL** — some sub-clauses met (the missing one is named in Status evidence below); **⬜ PLANNED** — forward-looking, not implemented yet. Baseline marks were verified against `master` @ `941e398cb266b29b88325a58605a16008d2af63c`; current-local exception [7] is explicitly branch-bound (see Status evidence).
...
| Local/test policy sync and native managed gateway | 🟡 PARTIAL [7] | Keep policy evaluation local during bounded control-plane loss while preserving executor-verifiable receipt provenance. | Authenticated strict-v2 sync, distinct physical publisher/attestation/decision keys, monotonic LKG cache, bounded degraded/expired behavior, native ALLOW once, and zero-side-effect negative cases. | Developers can exercise one real enrolled-gate → signed-policy → local-native-execution slice without a remote authorization call. | G203 still lacks the complete policy lifecycle dependency and bounded evidence-spool/capacity failure semantics; aggregate G017 also lacks persisted wiring attestation, fleet state, and canonical run evidence. |
...
6. **Policy (PARTIAL).** At the recorded master baseline, content-addressed policy **versioning** (`policy.py` `_compute_version` / `RuleSetPolicy`; CLI `policy inspect` / `policy export`; `test_policy_bundle_io.py`) and **tenant binding** (`tenant.py` active-bundle-by-tenant lookup; `test_tenant_safety.py`) shipped, while policy-bundle signatures and active/stale/revoked lifecycle states were missing. Current-local branch exception [7] adds signed publication and active-head advancement, but review, stale, superseded, revoked, diff, simulation, staged rollout, and rollback remain incomplete.
7. **Policy sync/native gateway (PARTIAL, current-local).** Branch `beta/p4-policy-sync-002` adds authenticated read-only policy-sync v2 in `packages/acgs-control-plane/src/acgs_control_plane/policy_sync.py`, strict local verification/LKG caching in `packages/gove-zone/src/gove_zone/policy_sync.py`, and managed native execution in `packages/gove-zone/src/gove_zone/gateway.py`. `test_policy_sync_routes.py`, `test_policy_sync.py`, and `test_policy_sync_integration.py` cover signed scope/provenance, independent physical key domains, cursor/ETag and high-water rollback behavior, bounded policy-cache outage handling, one-time ALLOW, and forbidden-side-effect failures. The cache and high-water floor are separately atomically replaced, not pairwise transactional; a high-water-first crash forces an authenticated full fetch and retains the rollback floor. The independent architect verdict is implementation-only GO. G203 remains incomplete because G202 lifecycle work and bounded evidence-spool/capacity failure semantics remain incomplete. Separately, aggregate G017 remains incomplete because G204 signed persisted wiring attestation, G205 operator fleet status, and canonical `run.json` are absent. This is not merged, deployed, production-ready, customer-used, or accepted managed evidence.
