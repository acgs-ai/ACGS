...
| Secure profile: signed receipts on by default | Close recomputed-hash residual for production-adjacent pilots. | Default secure profile rejects unsigned receipts; signed key registry tests pass. | Operators get safer defaults. | Unsigned dev mode may be misused as production assurance. |
...
| Reference integration: real agent platform (e.g. Mattermost Agents MCP) | Prove the MCP gateway against a widely-deployed agent surface, not just local examples. | A real platform's agent tool-calls (`create_post`, `create_channel`, file ops) route through the gate; fail-closed when no receipt; `expected_actor` bound to the platform's authenticated user; bypass-path documented. | Operators see the gate working on a production agent stack they recognise. | MCP-adapter conformance stays illustrative; adopters can't see a concrete end-to-end wiring. |
