...
- **需要重点投资（潜在差异来源）:** Decision Receipt 规范、签名、防篡改审计(Merkle / 链式)、replay、fail-closed 执行器语义、安全模型——这些是产品的核心，见 `docs/DECISION_RECEIPT_SPEC.md` / `docs/SECURITY_MODEL.md`；是否形成护城河仍需独立证据验证。
...
**潜在壁垒（待验证）:** 若证据规范被生态采纳，可能形成标准 / 网络效应；防篡改审计 + replay 只有在客户审计流程真实采用后，才可能形成嵌入式迁移成本。
