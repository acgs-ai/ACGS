...
| A2 | **Vendor dependency governance** | *Which third-party agent/tool acted, under whose authority, on what?* | Every side effect binds `actor`, `proposed_action`, `tenant_id`, `execution_boundary`, and an `argument_hash` into one hash-bound receipt. On a path wired through the gate, a vendor agent cannot act outside its issued receipt. `expected_actor` at the gate stops one caller replaying another's receipt. | `verify()` checks 2b (actor anchor), 5 (tenant), 6 (boundary), 10b (argument binding) |
