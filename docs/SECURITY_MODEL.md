...
| Native control-plane transaction drift | A managed route records a native receipt but commits a different SQL mutation, event/head, outbox row, consumption state, or idempotency replay result; treats `DENY`/`ESCALATE` as executable; or replays a key with a different semantic request. | The current agent-create route uses explicit distinct receipt issuer and consumption attestation providers; binds the full environment policy hash; records native receipt/event/head/outbox state, `ALLOW` consumption, and signed terminal idempotency result evidence in one SQL transaction; leaves `DENY`/`ESCALATE` non-executable; requires exactly one bounded `Idempotency-Key`; stores digest-only key/request/response evidence; and semantically replays `ALLOW`, `DENY`, or `ESCALATE` from authoritative database rows. PostgreSQL serializes through a tenant row lock; SQLite has same-process locking only. | `packages/acgs-control-plane/tests/test_native_agent_transaction_route.py`, `test_agent_create_idempotency.py`, `test_exports.py`, `test_native_receipt_ledger.py`, `test_receipts_dashboard.py`. | Only agent creation is on this canonical native path. Twelve legacy unsigned write aliases remain and still block production posture. Provider injection is explicit, but operator environment/provider wiring is incomplete. SQL atomicity and idempotency replay do not prove external exactly-once delivery. SQLite evidence is same-process only. Native project/environment scope is bound through the event path, not direct receipt columns. No other mutation, async export/recovery, rolling-upgrade, or production-readiness claim is made. | Complete native cutover, provider wiring, trust registry, cross-route idempotency, external delivery reconciliation, and production posture closure. |
| Action-tier downgrade | Agent declares the lenient `explore` tier to dodge strict `commit` policy rules or the gate. | Declared tier is untrusted; a tool-tier registry is authoritative (`effective = min(declared, registered)`; unregistered / no-registry ⇒ `commit`); tier is bound into `receipt_hash`. Policy-side enforcement applies at minting (a commit-only tool is evaluated under `commit`). Executor-side, an `explore` receipt is refused outright unless a tool-tier registry is supplied at the gate *and* marks the action explore-capable — no registry means no explore leniency. Receipt gate, `expected_actor`, audit append, and `DENY`/`ESCALATE` non-executability apply to every tier. | `test_action_tiering.py` (T1/T2/T3/T5b/T5c/T8). | Registry is manual/declarative — no semantic side-effect detection; operator must register side-effecting tools as `commit` (already the default). `ReceiptVerifier` does not thread a registry, so a receipt verified through it can never carry the explore tier. | Semantic tool classification, signed tier registry, and registry threading through `ReceiptVerifier`. |
...
| ADV6 | Supply-chain attacker | Poison a dependency, the build, or the constitution (policy bundle); per ADR-0010 D4 this includes dependency admission (a package-manager install or a direct manifest/lockfile edit) and lifecycle-script execution as named attack steps | Constitutional-hash CI + reproducible builds + binary transparency; at the integrated agent-hook boundary (ADR-0010), package installs receive explicit fail-closed decisions, lifecycle-script enablement is a separately recorded, escalated decision taken before the manager runs (it records the enablement decision, it does not gate the scripts), non-canonical package managers are denied, and dependency manifest/lockfile writes escalate to a human | G2 / Track A | [on-master, partial] | `.github/workflows/constitutional-hash.yml`, `tests/docs/` constitutional-hash checks; hook-boundary package governance in `packages/gove-zone/src/gove_zone/execution.py` with negative-path install, lifecycle, and manifest tests in `test_execution_governance.py`. Caveat below: the hash inventory is currently empty, so the gate is real plumbing over a no-op inventory; bit-reproducible build check is **[proposed]**; interactive and out-of-hook installs stay unmediated (the ADV9 residual) |
...
| ADV9 | **Out-of-gate executor bypass** (complete-mediation keystone) | Invoke the raw tool / side-effect path and never reach the governed executor; per ADR-0010 D4 the scope includes unmediated execution-environment paths: interactive shells, package managers, and developer tools writing tracked files | Adapter/gateway completeness + a static "is the gate wired" check; integration-matrix tiers per runtime; the execution-governance layer (ADR-0010, `packages/gove-zone/src/gove_zone/execution.py`) classifies hook-delivered shell commands by argv structure and returns fail-closed decisions with minted Decision Receipts via `handle_claude_hook` (the hook decides, the host performs the side effect; this is decision mediation, not receipt-gated execution) | G1 / Track F | [on-master, partial] | `test_integration_gaps.py`, `test_integration_hook.py`, `test_execution_governance.py`, `test_execution_bypass_adversarial.py`: *Executor bypass, MCP/tool-gateway misuse*. Residual: the kernel cannot stop code paths it is not wired into; interactive terminals and paths outside the hook remain unmediated, and the hook path does not call `execute_with_receipt`; gateway conformance is partial |
...
> **[on-master, partial]** controls (constitutional-hash CI plus hook-boundary
> package-manager governance per ADR-0010; the operator-supplied,
...
Every one of the 26 named threats in the table above is owned by at least one adversary. The
...
| Action-tier downgrade | ADV1 | ADV12 |
