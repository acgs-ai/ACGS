...
The kernel records the decision and issues a Decision Receipt. A missing, expired, mismatched, or unverifiable receipt is treated as DENY. ESCALATE receipts are likewise non-executable.
...
EN: ESCALATE is a kernel verdict. It is not a fourth kind of permission. The kernel records it, issues a receipt, and refuses the tool (fail closed). A later human ALLOW is a new proposed action, with a new receipt, or it is operator process — not this library pretending to be an approval UI.

ZH: ESCALATE 是一种内核判定。它不是第四种放行。内核记录它、发出收据、并拒绝工具（失败即关）。之后人的 ALLOW 是一次新的提议、一张新收据，或是运营过程 — 不是本库假装成人审界面。
...
| Row | Verdict | Notes |
|---|---|---|
| C1 | **Verified** | gove-zone README line 5 invariant matches. Binding proof is `examples/receipt-gated-execution/demo.py` in the standalone repo. Smoke is a shorter allow/deny/audit check and was not re-run this session. |
