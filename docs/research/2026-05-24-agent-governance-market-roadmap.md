...
|---|---|---|---|---|---|
| Root workspace | Governance code is spread across libraries, frontend, evals, bundles, and domain packs; developers need a single source of truth. | Keep the parent repo as an honest registry and verification fan-out, not a confused mega-package. | Dependency/CI routing as governance: every package has a declared gate and evidence owner. | uv workspace, Python 3.11 local floor, pnpm 9.15/Node 24 for frontend, path-filtered GitHub Actions, Makefile fan-out. | `docs/governance-stack-index.md` now maps each package to its policy/evidence contract, local gate, and live/deploy-proof caveat, with `scripts/check_governance_stack_index.py` guarding required rows, ignored adjacent-checkout caveats, and overclaim terms. The root registry now separates ignored LegalGuard/legal-skill checkouts from parent-tracked packages and gives the enterprise admin adjunct a concrete build gate. Next: collect credentialed deploy/browser/API proof for the console/analyzer path. |
| `packages/gove-zone/` | Agents can execute side-effectful tool calls before governance/audit happens. | Minimal runtime kernel that denies when policy/audit/receipt fails; keep LOC pressure visible while policy depth grows. | Implement Policies-on-Paths in the kernel decision request; treat tool dispatch as syscall mediation. | Python 3.11, stdlib, optional Pydantic, SHA-256 JSONL chain, `fcntl.flock`, CLI hooks, future Rust/WASM sidecar. | Initial path-aware dispatch, reviewable rule bundles, and local fixture evaluation are now in-kernel: `PathBoundaryPolicy`, `RuleSetPolicy`, actor/path/state-hash audit fields, `gove-zone policy inspect/export`, `gove-zone eval`, and tests prove protected-path denial before execution. Eval reports now ingest into `acgs_governance_eval_mvp` as hash-addressed audit evidence via API/CLI; the eval MVP exposes tenant-scoped `/evidence/evaluation-reports` queries, and the console now has a guarded `/console/audit/:receiptId` proof journey backed by bus-owned `GET /api/bus/receipts/{receipt_id}` proof packets. The bus analyzer now accepts canonical `gove-zone` audit-tail records through `agent-bus-analyzer import-audit`/observer projection, preserving `tool`, `argument_hash`, `timestamp_iso`, and `event_hash` into receipt proof packets; receipt exports carry either explicit local digest metadata or deployment-managed HMAC signature metadata, and the console surfaces that status/key id to operators. The analyzer API now has a store-backed serve path, Cloud Run/Secret Manager deployment contract for required signing, Cloud Storage FUSE trace bucket mount guarded to one file-store writer, a one-shot import Job template, and a signed-proof postdeploy smoke CLI. AgentDojo/InjecAgent/ToolEmu-style fixture adapters now load local benchmark-shaped JSON through `gove-zone eval --benchmark-format agentdojo|injecagent|toolemu`, preserving attack/utility metrics and high-stakes tool hazard tags without claiming full upstream-suite execution. Next: collect deployed-browser/API proof from a real rendered analyzer revision. |
...
| `packages/agent-bus-analyzer/` | Even when a gate exists, operators cannot see dispatch/response/event drift and tampering across the bus. | Observer-only analysis API that never authorizes but detects wiring defects, gaps, tampering, and stale evidence. | Evidence graph: chain-hashed bus events + OpenAPI projection + console trace lens. | Python 3.11, FastAPI, Pydantic, anyio, uvicorn, JSON schema, mypy/ruff. | Receipt lookup now returns console-ready proof packets, can be backfilled from canonical `gove-zone` audit JSONL via `import-audit`, and supports `ACGS_EVIDENCE_SIGNING_*` HMAC signing with fail-closed required mode; the serve command now accepts `--store-dir`/`AGENT_BUS_ANALYZER_STORE_DIR`, with a non-root Dockerfile and Cloud Run template using a pinned Secret Manager `secretKeyRef` for `ACGS_EVIDENCE_SIGNING_SECRET`. The template now mounts a Cloud Storage FUSE trace bucket at `/var/lib/agent-bus-analyzer` with stable UID/GID `10001` and `maxScale=1` while `TraceStore` remains SQLite+JSONL file-backed; a companion Cloud Run Job template imports one canonical audit JSONL from the mounted bucket, and `postdeploy-smoke` verifies health plus a deployment-signed receipt proof. Receipt events now also propagate Phoenix/OpenTelemetry cross-links (`phoenix_trace_id`, `phoenix_span_id`, `phoenix_parent_span_id`) from `traceparent`/explicit carrier fields into the signed proof packet and OpenAPI surface. Next: run the smoke against a real rendered revision after credentialed deploy. |
| `acgi-ai/` | Buyers and auditors cannot trust invisible backend claims; they need a polished, privilege-aware evidence console. | Buyer-demoable public/trust pages plus privileged console where one policy enforcement decision can be verified and exported. | Turn receipts into buyer-facing artifacts: policy version, actor, reason, hash, trace, replay status, export. | React 19, Vite, TypeScript 6, Tailwind 4, TanStack Router/Query, Biome, MSW, OpenAPI types, Cloud Run console, Vercel marketing, strict CSP. | The `/console/audit/:receiptId` proof journey now loads one receipt proof packet, prefers the bus-owned `/api/bus/receipts/{receipt_id}` lookup, shows hash-chain status, Phoenix trace/span ids when present, policy/path/action, before/after state, signature status/algorithm/key id/digest, and offers evidence packet download. The audit page also renders source-aware evaluation evidence cards for gove-zone, local AgentDojo-style, local InjecAgent-style, and local ToolEmu-style high-stakes report sources with explicit claim-safe/local-evidence caveats. Next: point `CONSOLE_BUS_UPSTREAM` at the rendered analyzer/bus gateway, add deployed-browser screenshot evidence for the proof journey, and verify live signed packets from the managed service. |
| `acgs_governance_eval_mvp/` | Governance claims need benchmarks; otherwise “safe agent” is just marketing copy. | Evaluation harness for authority, policy recall, governance recall, audit, replay, API, and OTel metrics. | Convert AgentDojo/InjecAgent/ToolEmu into regression gates and market-facing metrics. | Python 3.11, FastAPI optional, OTel optional, JSONL chain, pytest. | `governance.evaluation` ingests `gove-zone eval` JSON, hashes normalized reports, appends chain-hashed audit evidence, marks failed reports unusable for claims, lists normalized event-hash-addressed reports through `/evidence/evaluation-reports?status=passed`, and is now consumed by the console audit API client contract. `governance.benchmarks.agentdojo_adapter`, `injecagent_adapter`, and `toolemu_adapter` convert local benchmark-result fixtures into source-tagged, claim-safe report evidence, including high-stakes tool hazard tags. The console maps those source-tagged reports into source-aware evidence-card copy with local-evidence caveats. Next: add live API/browser proof for source-tagged report cards. |
...
- AgentDojo/InjecAgent/ToolEmu local fixture adapters;
- ToolEmu-style synthetic high-stakes tool cases with hazard tags;
