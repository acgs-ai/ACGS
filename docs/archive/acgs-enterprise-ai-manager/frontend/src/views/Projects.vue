<template>
  <div class="projects-view">
    <div class="page-header">
      <h2>Projects Management</h2>
      <button @click="showCreateForm = true" class="btn btn-primary">Create Project</button>
    </div>

    <div v-if="error" class="error">{{ error }}</div>

    <div v-if="showCreateForm" class="card">
      <h3>{{ editingProject ? 'Edit Project' : 'Create New Project' }}</h3>
      <form @submit.prevent="saveProject">
        <div class="form-group">
          <label>Project Name</label>
          <input v-model="form.name" type="text" required />
        </div>
        <div class="form-group">
          <label>Description</label>
          <textarea v-model="form.description"></textarea>
        </div>
        <div class="form-group">
          <label>Status</label>
          <select v-model="form.status">
            <option value="planning">Planning</option>
            <option value="active">Active</option>
            <option value="on_hold">On Hold</option>
            <option value="completed">Completed</option>
          </select>
        </div>
        <div class="form-actions">
          <button type="submit" class="btn btn-success">Save</button>
          <button type="button" @click="cancelForm" class="btn">Cancel</button>
        </div>
      </form>
    </div>

    <div v-if="loading" class="loading">Loading projects...</div>

    <table v-else-if="projects.length > 0" class="table">
      <thead>
        <tr>
          <th>Name</th>
          <th>Status</th>
          <th>Created</th>
          <th>Actions</th>
        </tr>
      </thead>
      <tbody>
        <tr v-for="project in projects" :key="project.id">
          <td>{{ project.name }}</td>
          <td><span :class="'badge badge-' + project.status">{{ project.status }}</span></td>
          <td>{{ formatDate(project.created_at) }}</td>
          <td>
            <button @click="editProject(project)" class="btn btn-sm">Edit</button>
            <button @click="deleteProject(project.id)" class="btn btn-sm btn-danger">Delete</button>
          </td>
        </tr>
      </tbody>
    </table>

    <div v-else class="card">
      <p class="empty-state">No projects found.</p>
    </div>
  </div>
</template>

<script setup>
import { ref, onMounted } from 'vue'
import apiClient, { listItems } from '@/api/client'

const projects = ref([])
const loading = ref(false)
const error = ref('')
const showCreateForm = ref(false)
const editingProject = ref(null)
const form = ref({ name: '', description: '', status: 'planning' })

const fetchProjects = async () => {
  loading.value = true
  error.value = ''
  try {
    const response = await apiClient.get('/projects')
    projects.value = listItems(response.data)
  } catch (err) {
    error.value = 'Failed to load projects'
    console.error(err)
  } finally {
    loading.value = false
  }
}

const saveProject = async () => {
  try {
    if (editingProject.value) {
      await apiClient.put(`/projects/${editingProject.value.id}`, form.value)
    } else {
      await apiClient.post('/projects', form.value)
    }
    await fetchProjects()
    cancelForm()
  } catch (err) {
    error.value = 'Failed to save project'
    console.error(err)
  }
}

const editProject = (project) => {
  editingProject.value = project
  form.value = { ...project }
  showCreateForm.value = true
}

const deleteProject = async (id) => {
  if (!confirm('Are you sure?')) return
  try {
    await apiClient.delete(`/projects/${id}`)
    await fetchProjects()
  } catch (err) {
    error.value = 'Failed to delete project'
    console.error(err)
  }
}

const cancelForm = () => {
  showCreateForm.value = false
  editingProject.value = null
  form.value = { name: '', description: '', status: 'planning' }
}

const formatDate = (date) => {
  return new Date(date).toLocaleDateString()
}

onMounted(() => {
  fetchProjects()
})
</script>

<style scoped>
.page-header {
  display: flex;
  justify-content: space-between;
  align-items: center;
  margin-bottom: 2rem;
}

.form-actions {
  display: flex;
  gap: 1rem;
  margin-top: 1rem;
}

.btn-sm {
  padding: 0.5rem 1rem;
  font-size: 0.875rem;
  margin-right: 0.5rem;
}

.badge {
  padding: 0.25rem 0.75rem;
  border-radius: 12px;
  font-size: 0.875rem;
  font-weight: 500;
}

.badge-planning { background: #95a5a6; color: white; }
.badge-active { background: #3498db; color: white; }
.badge-on_hold { background: #f39c12; color: white; }
.badge-completed { background: #2ecc71; color: white; }
</style>
