...
| Signing mode | `packages/gove-zone/src/gove_zone/signing.py` | Ed25519 signing for receipts. Issuance is unsigned unless a signer is configured; the default gate requires a verified signature (`require_signature=True`) and fails closed without one, so unsigned operation requires explicitly selecting dev mode. |
