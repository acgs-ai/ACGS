...
- ✅ **Inverted parity solved structurally.** For a governance layer the parity discipline is *every side-effecting tool routes through the gate*. The kernel enforces this by construction for tools registered through it: a registered tool has no un-gated execution path. A side effect an agent reaches *without* calling a registered tool is outside the kernel's mediation — see `docs/ENFORCEMENT-BOUNDARY.md`.
...
2. **ESCALATE is resumable, and not bypassable through the gate.** A test suite proves: approve→resume executes exactly once; resume without a fresh valid receipt fails closed; approver == proposer fails closed (MACI). Re-running the original call without a new receipt does **not** execute. (R2/R6)
...
5. **Every guarded tool is structurally gated.** A dispatcher-level test (per `~/.claude/rules/review-handler-wiring.md`) shows each production MCP tool routes through the gate; a new tool added without registration is *not callable through the dispatcher at all*, rather than callable-if-the-dev-remembered-to-call-admit. This closes admission-by-omission, not in-process bypass. (R5)
