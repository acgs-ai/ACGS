...
- `ACGS/packages/acgs-lite/src/acgs_lite/integrations/hermes.py` already defines `HermesGovernor.evaluate(tool_name, tool_input, session_meta)`, fails closed on evaluation and audit errors, raises `GovernanceViolationError` on denial, and checks schema, protected paths, secrets, destructive commands, shell tools, workspace writes, network allowlists, and `GovernanceEngine` violations.
- `ACGS/.hermes/governance/hermes.constitution.yaml` already defines HRM-001 through HRM-008 for shell confirmation, workspace write boundary, secret denial, destructive patterns, network allowlist, read audit, protected governance assets, and required tool names.
...
- `ACGS/packages/acgs-lite/src/acgs_lite/traces/schema.py` already defines `GovernedTraceBundle` with proposed action, tool name, policy snapshot hash, decision, execution status, execution result hash, previous bundle hash, bundle hash, replay metadata, and verifier results.
