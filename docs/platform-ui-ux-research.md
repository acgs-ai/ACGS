...
21. **Hosted Storybook runway.** Hosted buyer-evidence should have a visible publish sequence before anyone removes the blocker. The UI should show Build local gallery, Enable Pages deploy, Build proof gap report, Verify live Storybook, and Attach hosted proof with the `storybook:build`, `STORYBOOK_PAGES_ENABLED=true`, `hosted-storybook-proof-gap-report.json`, `storybook-manifest-live`, and `copyIntoProductionEvidence.hostedStorybook` labels while keeping the boundary clear that local runway guidance is not hosted proof.
...
- Add a hosted Storybook runway under the same launch proof ladder so the buyer-evidence path is visible as **Build local gallery → Enable Pages deploy → Build proof gap report → Verify live Storybook → Attach hosted proof** before `hosted-storybook-buyer-evidence` can be cleared.
