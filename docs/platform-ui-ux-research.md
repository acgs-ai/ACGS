...
17. **Production command rail.** Deploy operators should not have to reconstruct proof commands from docs. The UI should show make production-blocker-evidence, verify:production-live, validate:production-evidence, and validate:hosted-storybook-proof with the artifact each command produces.
18. **Assurance proof intake.** External proof should have an operator-facing intake map before launch. The UI should show Production authority, Legal claim review, Security assessment, Manual accessibility, and Hosted buyer evidence lanes with the required template or assurance field and an explicit reminder that placeholders are not proof.
