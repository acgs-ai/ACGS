...
# ---------------------------------------------------------------------------
# Cross-instance process-level lock registry
# ---------------------------------------------------------------------------
# Multiple ChainEvidenceWriter instances that target the same JSONL file must
# share one threading.Lock so they cannot interleave their load-tail/append
# sequences.  We keep a module-level dict (path → Lock) guarded by its own
# meta-lock.
...
# ---------------------------------------------------------------------------
# Optional cross-process file lock (fcntl on POSIX; no-op elsewhere)
# ---------------------------------------------------------------------------
try:
    import fcntl as _fcntl

    def _flock_exclusive(fh: Any) -> None:  # type: ignore[misc]
        _fcntl.flock(fh, _fcntl.LOCK_EX)

    def _flock_release(fh: Any) -> None:  # type: ignore[misc]
...
class ChainEvidenceWriter:
    ...
    def append_event(
        ...
        *,
        ...
        Safe for concurrent calls from multiple threads **and** from multiple
        processes (POSIX only for the cross-process guarantee).  The
        load-tail/write sequence is protected by both an in-process Lock shared
        across all instances that point to the same file, and an exclusive
        fcntl file lock on a companion ``.lock`` file.
        """
        with self._lock:
            with self._lock_path.open("w") as _lock_fh:
                _flock_exclusive(_lock_fh)
                try:
                    # Re-read the real tail from disk so that a concurrent writer
                    # in another process cannot cause a prev_hash collision.
                    prev_hash = self._load_previous_hash()
                    body = GovernEvent(
                        id=str(uuid.uuid4()),
                        ts=utc_now_iso(),
