...
| `gap_started_at` | `datetime \| None` | Populated only when `status="ingest-gap"`. |
| `gap_ended_at` | `datetime \| None` | Populated only when `status="ingest-gap"`. |
...
ingest-gap        — synthetic marker for a window the analyzer could not capture (excluded from chain + SC-002 metrics)
```
...
- Events with `status="ingest-gap"` MUST NOT participate in the hash chain. Chain verification (`_verify_chain` in `store.py`) skips these rows; the `prev_hash` of the event immediately after a gap references the `event_hash` of the event immediately before the gap.
- Classification accuracy metrics (SC-002) are computed against events with `status IN (completed, policy-violation, dispatch-failure, unwired-handler, orphan-response, incomplete-pair)`; `ingest-gap` rows are excluded from the denominator.
