---
name: maintain-acgs
description: Manual ACGS workspace maintenance checklist for boundaries, gates, sealed files, and Claude automation self-checks.
...
Checklist:
1. Load the nearest `AGENTS.md` / `CLAUDE.md` for every directory you inspect or modify.
...
5. For `.claude` maintenance, run automation self-checks:
   - `python3 -c 'import json, pathlib; json.load(open(pathlib.Path(".claude/settings.json")))'`
