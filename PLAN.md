...
| File | Purpose |
|---|---|
| `acgi-ai/src/lib/session.ts` | sessionStorage-backed session gate + `SESSION_CHANGE_EVENT`. Wires Login → Console redirect via `?next=`. |
| `acgi-ai/src/routes/ProductSurfaces.tsx` | `/products` index + `/products/<slug>` for the 6 product reference pages (LegalGuard, Governance Eval, ACGS Lite, Hermes, EU AI Act, Vault Demo). |
| `acgi-ai/src/routes/console/shared.tsx` | `useTextFilter`, `SearchToolbar`, `Receipt`, `EmptyState`, `ConsoleLoading`, `ConsoleError` — the state-coverage primitives Phase 1 will adopt across every page. |
| `acgi-ai/src/mocks/data/console-summary.ts` | Console summary fixture used by `useConsoleSummary`. |
| `acgi-ai/src/vite-env.d.ts` | Vite env-var type declarations. |
| `acgi-ai/tailwind.theme.json` + `acgi-ai/tokens.json` | Design token exports (Tailwind + DTCG) generated by `pnpm design:export`. |
| `acgi-ai/.node-version` | Pin Node 24 for direnv / asdf / fnm consumers. |
| `acgi-ai/scripts/*` | Token check + smoke scripts (`check-design-tokens.py`, `smoke-internal-doc-deny.sh`, etc.). |
| Per-directory `acgi-ai/**/AGENTS.md` | Contributor docs for future humans + Claudes. Already landed in similar form in other repo subtrees. |

### Phase 0b — Modifications to existing master files (~30 M files)

| Cluster | Files | Nature |
|---|---|---|
| Router gating | `src/App.tsx`, `src/main.tsx` | Wire `hasSession()` from new `lib/session.ts`; add `/login`, `/privacy`, `/products` dispatch. |
| Surface polish | `src/routes/{Console,Marketing,Login}.tsx`, all `src/routes/console/*.tsx` | Adopt `shared.tsx` primitives where present, copy edits, citation refresh. |
| API layer polish | `src/api/{client,hooks,types}.ts`, `src/mocks/handlers.ts` | Type tightening + endpoint additions consistent with current dirty state. |
| Styling | `src/App.css`, `src/index.css` | Token additions for new product surfaces; CSP-safe class additions. |
| Config | `acgi-ai/package.json`, `acgi-ai/vite.config.ts`, `acgi-ai/.gitignore` | Token-export scripts, biome config refinements. |
| Infra | `acgi-ai/infra/{Caddyfile,Dockerfile.console}`, `acgi-ai/.github/workflows/{console,marketing}.yml`, `acgi-ai/vercel.json` | Header additions, internal-doc denial smoke wiring. |
| Contracts | `acgi-ai/DESIGN.md`, `acgi-ai/DEPLOY.md` | Contract refinements (continue to be authoritative). |

### Phase 0 deliverable

These changes split into 3-4 narrow PRs from `chore/eval-regression-coverage-seed-harness`
(or a new feature branch carved from it) into master, then `plan/complete-platform-frontend`
rebased on top. Suggested PR boundaries:

1. **State utilities + product surfaces** — `lib/session.ts`, `ProductSurfaces.tsx`,
   `console/shared.tsx`, related router gating in `App.tsx` and `main.tsx`.
2. **Surface polish + token exports** — bulk modifications to `routes/`, css, design exports.
3. **Infra polish** — Caddyfile / Dockerfile / workflows / vercel.json modifications, scripts dir.
4. **Contributor docs** — per-directory `AGENTS.md` files (low risk, can land last).

**Risk if skipped:** Phase 1's "audit all 11 console pages against `shared.tsx`" depends on
`shared.tsx` being on master. Phase 5's deploy verification depends on the infra modifications.
Without Phase 0 landed, the rest of the plan references files that exist only on a stale
unrelated branch.
...
| 2026-05-05 | WIP frontend deltas (3 net-new utility files + ~30 polish modifications) are Phase 0 inheritance, split into 3-4 narrow PRs | Bulk of the architecture (api layer, MSW, console pages, infra) already on master in `c611924`/`899b666`. Inheritance scope is much smaller than initial plan claimed. |
