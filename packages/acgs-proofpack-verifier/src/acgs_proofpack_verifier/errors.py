...
:class:`~acgs_proofpack_verifier.decision.DecisionRecord` and the audit chain hash that
...
from acgs_proofpack_verifier.authz import AuthzReason
from acgs_proofpack_verifier.decision import DecisionRecord
from acgs_proofpack_verifier.rejection import HUMAN_APPROVAL, REVISE_AND_RETRY, rejection_dict

if TYPE_CHECKING:
    from acgs_proofpack_verifier.escalation import PendingApproval
...
class ReceiptRejectionReason(StrEnum):
    ...
    :meth:`acgs_proofpack_verifier.receipt.DecisionReceipt.verify` rejection and every
    ...
    # multi-agent governance DAG reasons (acgs_proofpack_verifier.dag)
...
class ProductionProfileError(ReceiptValidationError):
    ...
    ``ReceiptValidationError`` (e.g.
    :meth:`acgs_proofpack_verifier.contracts.ReceiptVerifier.is_valid`)
...
class ReceiptAlreadyUsedError(ReceiptValidationError):
    ...
    A :class:`~acgs_proofpack_verifier.consumption.ReceiptConsumptionLedger` keys consumption
...
class IdentityRejectionReason(StrEnum):
    ...
    :class:`~acgs_proofpack_verifier.authz.AuthzReason`. Values equal member names (StrEnum)
...
class IdentityError(GoveZoneError):
    ...
    Covers the Identity/Authority layers (:mod:`acgs_proofpack_verifier.identity`): unknown or
...
class AuthzDeniedError(GoveZoneError):
    ...
    :class:`~acgs_proofpack_verifier.authz.AuthzReason` taxonomy the kernel emits as
    ``AUTHZ_DENY:<reason>``.
    """
...
class DeniedError(GoveZoneError):
    ...
    def to_rejection_dict(
        ...
        *,
        ...
        the deciding record — see :func:`acgs_proofpack_verifier.rejection.rejection_dict`.

        ``allowed_alternatives`` (from
        :func:`acgs_proofpack_verifier.rejection.discover_alternatives`) is passed through to
...
class EscalateError(GoveZoneError):
    ...
    ``pending`` carries the :class:`~acgs_proofpack_verifier.escalation.PendingApproval` needed
    ...
    def to_rejection_dict(
        ...
        *,
        ...
        (:func:`acgs_proofpack_verifier.escalation.approve_escalation` →
        :func:`acgs_proofpack_verifier.escalation.resume_with_receipt`). ``resumable`` tracks
        the actual affordance — it is ``True`` **iff** a
        :class:`~acgs_proofpack_verifier.escalation.PendingApproval` is attached (the kernel
