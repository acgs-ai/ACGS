...
:meth:`~acgs_proofpack_verifier.receipt.DecisionReceipt.verify` is stateless: it proves a
...
:func:`~acgs_proofpack_verifier.executor.execute_with_receipt` re-runs the side effect every
...
that anchored the deciding :class:`~acgs_proofpack_verifier.decision.DecisionRecord`. Every
:meth:`~acgs_proofpack_verifier.audit.ChainHashAuditStore.append` produces a unique
...
* **Burn-before-execute.** The ledger entry is fsync'd to disk under an
  ...
  gets :class:`~acgs_proofpack_verifier.errors.ReceiptAlreadyUsedError` before any side
  effect.
...
* **An unreadable or corrupt ledger refuses execution.** If the ledger cannot
  ...
  (:class:`~acgs_proofpack_verifier.errors.ConsumptionLedgerError`).
...
receipt fails :meth:`~acgs_proofpack_verifier.receipt.DecisionReceipt.verify` check 13 *before*
...
from acgs_proofpack_verifier._locking import _exclusive_file_lock
from acgs_proofpack_verifier.audit import GENESIS_HASH
from acgs_proofpack_verifier.decision import sha256_json
...
# failed verify/reconcile). Mirrors ``acgs_proofpack_verifier.integration`` — a *logger record
...
_LOGGER = logging.getLogger("acgs_proofpack_verifier.consumption")
...
class ReceiptConsumptionLedger:
    ...
    :class:`~acgs_proofpack_verifier.audit.ChainHashAuditStore` (``fcntl.flock`` on POSIX,
    ...
    def observability(self) -> LedgerObservability:
        ...
        at zero (the durable, fleet-wide surface is the ``acgs_proofpack_verifier.consumption``
    ...
    def consume(self, receipt: DecisionReceipt) -> dict[str, Any]:
        ...
        :class:`~acgs_proofpack_verifier.errors.ReceiptAlreadyUsedError` if the receipt's
        audit anchor is already in the ledger,
        :class:`~acgs_proofpack_verifier.errors.ReceiptValidationError` if the receipt
        carries no audit anchor to key on, and
        :class:`~acgs_proofpack_verifier.errors.ConsumptionLedgerError` if the ledger cannot
    ...
    def is_consumed(self, audit_event_hash: str) -> bool:
        ...
        surface as a transient :class:`~acgs_proofpack_verifier.errors.ConsumptionLedgerError`.
        """
    ...
    def _iter_records(self) -> Iterator[tuple[int, dict[str, Any]]]:
        ...
        every reader surfaces the same
        :class:`~acgs_proofpack_verifier.errors.ConsumptionLedgerError` taxonomy
        on a corrupt ledger rather than leaking a raw decode error.
        """
    ...
    def _scan_for_consume(self, key: str) -> tuple[bool, str]:
        ...
        burn onto, or :data:`~acgs_proofpack_verifier.audit.GENESIS_HASH` when the file is
    ...
    def _parse_aware_ts(value: str) -> datetime | None:
        ...
        Mirrors the contract of :meth:`~acgs_proofpack_verifier.receipt.DecisionReceipt.verify`
    ...
    def prune(self, *, now: datetime | None = None) -> dict[str, Any]:
        ...
        *now* defaults to the real UTC wall clock and must be timezone-aware. Pass
        ...
        :class:`~acgs_proofpack_verifier.errors.ConsumptionLedgerError` (same fail-closed
    ...
    def verify_ledger(self, expected_last_hash: str | None = None) -> dict[str, Any]:
        ...
        Twin of :meth:`~acgs_proofpack_verifier.audit.ChainHashAuditStore.verify_chain`,
        ...
        :class:`~acgs_proofpack_verifier.errors.ConsumptionLedgerError` (same contract as
        :meth:`_scan_consumed`).
        """
        previous = GENESIS_HASH
        checked = 0
    ...
    def reconcile(self, audit_store: ChainHashAuditStore) -> dict[str, Any]:
        ...
        *audit_store* is duck-typed — any object exposing
        ...
        passes a :class:`~acgs_proofpack_verifier.audit.ChainHashAuditStore`). It is supplied
