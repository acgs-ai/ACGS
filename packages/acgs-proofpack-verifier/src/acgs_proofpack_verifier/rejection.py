...
(:class:`~acgs_proofpack_verifier.errors.DeniedError` /
:class:`~acgs_proofpack_verifier.errors.EscalateError`)
carrying the deciding :class:`~acgs_proofpack_verifier.decision.DecisionRecord`. This module
projects that record into a small, stable JSON envelope a *calling agent* can
read to self-correct — the agent-facing twin of
:func:`acgs_proofpack_verifier.frontend_contract.record_to_governed_action`, which targets the
...
read-only :meth:`~acgs_proofpack_verifier.kernel.Kernel.simulate` primitive to fill
...
from acgs_proofpack_verifier.decision import Decision, DecisionRecord
...
class SupportsSimulate(Protocol):
    ...
    read-only :meth:`acgs_proofpack_verifier.kernel.Kernel.simulate` signature qualifies.

    Declared structurally (not as ``Kernel``) so this module keeps importing
    only :mod:`acgs_proofpack_verifier.decision`: ``errors`` imports this module and
...
def alternative_from_record(record: DecisionRecord) -> dict[str, Any]:
    ...
    :meth:`~acgs_proofpack_verifier.kernel.Kernel.simulate`) into an ``allowed_alternatives``
    entry.
...
def discover_alternatives(
    ...
    :meth:`~acgs_proofpack_verifier.kernel.Kernel.simulate` — runs a read-only simulation and
    ...
    *prediction* under the current policy, not authorization — execution still
    ...
    :class:`~acgs_proofpack_verifier.errors.UnknownToolError` (mirroring ``simulate``) —
...
if TYPE_CHECKING:
    ...
    from acgs_proofpack_verifier.kernel import Kernel
