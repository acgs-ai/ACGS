...
def test_signed_receipt_verifies_and_executes() -> None:
    ...
    side_effect = SideEffect()
    result = _run_gate(receipt, side_effect, verifier=verifier, require_signature=True)
    assert result == "SIDE EFFECT EXECUTED"
    assert side_effect.ran is True


def test_forged_recomputed_receipt_rejected_without_private_key() -> None:
    ...
    signer = Ed25519Signer.generate()
    verifier = Ed25519Signer.from_public_bytes(signer.public_bytes())
    receipt = _issue(signer, actor="agent-1")
    ...
    with pytest.raises(ReceiptValidationError, match="invalid signature"):
        _run_gate(forged, side_effect, verifier=verifier, require_signature=True)
    assert side_effect.ran is False


def test_attacker_mints_with_own_keypair_unknown_key_rejected() -> None:
    ...
    forged = _issue(attacker)  # internally valid, signed by the attacker's key
    ...
    with pytest.raises(ReceiptValidationError, match="unknown signing key"):
        _run_gate(forged, side_effect, verifier=registry, require_signature=True)
    assert side_effect.ran is False


def test_signed_receipt_via_known_key_mapping_executes() -> None:
    ...
    side_effect = SideEffect()
    _run_gate(receipt, side_effect, verifier=registry, require_signature=True)
    assert side_effect.ran is True


def test_algorithm_downgrade_rejected() -> None:
    ...
    with pytest.raises(ReceiptValidationError, match="unsigned receipt rejected"):
        _run_gate(downgraded, side_effect, verifier=verifier, require_signature=True)
    assert side_effect.ran is False
...
def test_wrong_public_key_rejected() -> None:
    ...
    side_effect = SideEffect()
    with pytest.raises(ReceiptValidationError, match="invalid signature"):
        _run_gate(receipt, side_effect, verifier=wrong_verifier, require_signature=True)
    assert side_effect.ran is False


def test_unsigned_receipt_allowed_when_not_required() -> None:
    """Backward compatibility: unsigned receipt (signer=None) executes when
    require_signature is False, with no verifier configured.
    """
    receipt = _issue(None)
    ...
    side_effect = SideEffect()
    _run_gate(receipt, side_effect, verifier=None, require_signature=False)
    assert side_effect.ran is True
...
def test_signing_fields_bound_into_receipt_hash() -> None:
    ...
    with pytest.raises(ReceiptValidationError, match="receipt_hash mismatch"):
        _run_gate(tampered, side_effect, verifier=verifier, require_signature=True)
    assert side_effect.ran is False
    ...
    with pytest.raises(ReceiptValidationError, match="receipt_hash mismatch"):
        _run_gate(tampered2, side_effect2, verifier=verifier, require_signature=True)
    assert side_effect2.ran is False


def test_signature_algorithm_mismatch_rejected() -> None:
    ...
    with pytest.raises(ReceiptValidationError, match="signature algorithm mismatch"):
        _run_gate(receipt, side_effect, verifier=NoneAlgVerifier(), require_signature=True)
    assert side_effect.ran is False


def test_signed_receipt_without_verifier_rejected() -> None:
    ...
    side_effect = SideEffect()
    with pytest.raises(
        ReceiptValidationError, match="signed receipt requires a configured verifier"
    ):
        _run_gate(signed, side_effect, verifier=None, require_signature=False)
    assert side_effect.ran is False


def test_governed_executor_enforces_require_signature() -> None:
...
def test_receipt_verifier_threads_signature_params() -> None:
