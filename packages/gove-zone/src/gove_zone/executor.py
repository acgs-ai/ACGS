...
def execute_with_receipt(
    ...
    **Production profile is the default.** ``require_signature`` defaults to
    ...
    now carries the full 64-hex SHA-256 of its canonical bundle, so this binds at
    full SHA-256 second-preimage strength. Truncated identities are rejected at
    construction and are never accepted here — there is no dual-acceptance path.
