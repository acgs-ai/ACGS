    ...
    for e in existing:
        if e.get("authority_evidence_id") == record["authority_evidence_id"]:
            if e.get("source_digest") == record["source_digest"]:
                print(f"IDEMPOTENT: {record['authority_evidence_id']} already ingested; no change")
                return 0
            print(
                f"REJECTED: {record['authority_evidence_id']} already exists with a different "
                f"source_digest — one logical authority fact only",
                file=sys.stderr,
            )
            return 3
