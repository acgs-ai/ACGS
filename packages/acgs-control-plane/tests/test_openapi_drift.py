    ...
    "/orgs/{org_id}/agents": {
        ...
        "post": {
            ...
            "parameters": _expected_params(
                "path:org_id", "header:X-API-Key", "header:Idempotency-Key:required"
            ),
            "responses": ["201", "409", "422", "428"],
            "tag": "agents",
        },
    },
...
def test_current_openapi_contract_records_missing_beta_contract_boundaries(
    ...
    # The platform tenant-bootstrap response component records the consumed
    # idempotency_key; outside that one route and component no snake_case
    # idempotency surface may appear in the schema. The exact header
    # parameters per path are pinned by EXPECTED_PATHS in the contract test
    # above, so an unexpected Idempotency-Key placement is caught there.
