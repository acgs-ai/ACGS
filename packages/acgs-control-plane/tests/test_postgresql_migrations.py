...
def _head_schema_with_unsupported_object_snapshot() -> tuple[tuple[object, ...], ...]:
    ...
    assert inspect_schema(test_url).state is DatabaseSchemaState(f"version_{HEAD_REVISION}")
    return before
...
def test_postgresql_clean_install_has_types_and_cross_org_parent_constraint() -> None:
    ...
    assert result.after.state is DatabaseSchemaState(f"version_{HEAD_REVISION}")
    assert inspect_schema(_TEST_POSTGRES_URL).state is DatabaseSchemaState(
        f"version_{HEAD_REVISION}"
    )
...
def test_application_refuses_shadow_first_search_path_before_serving_or_mutation(
    ...
    assert inspect_schema(hostile_url).state is DatabaseSchemaState(f"version_{HEAD_REVISION}")
...
def _exercise_forced_termination_rollback_and_lock_release(
    ...
    *,
    ...
    assert owner_ready["transaction_state"] == DatabaseSchemaState(f"version_{HEAD_REVISION}").value
...
def _drop_post_0001_tables(connection: Connection) -> None:
    ...
    for function_name in (
        ...
        connection.execute(sa.text(f"DROP FUNCTION IF EXISTS {function_name}() CASCADE"))
...
def test_postgresql_exact_head_production_is_blocked_before_persistence_and_local_is_not_ready(
    ...
    try:
        ...
        assert response.json() == {
            ...
            "schema_current": True,
            "schema_state": DatabaseSchemaState(f"version_{HEAD_REVISION}").value,
        }
        assert not audit_dir.exists()
    finally:
        app.state.engine.dispose()
