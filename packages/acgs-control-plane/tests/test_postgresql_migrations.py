...
def _head_schema_with_unsupported_object_snapshot() -> tuple[tuple[object, ...], ...]:
    ...
    assert inspect_schema(test_url).state is DatabaseSchemaState.VERSION_0006
    return before
...
def test_application_refuses_shadow_first_search_path_before_serving_or_mutation(
    ...
    assert inspect_schema(hostile_url).state is DatabaseSchemaState.VERSION_0006
...
def _assert_success_event(worker: _MigrationWorker, event: dict[str, object]) -> int:
    assert event == {
        ...
        "after": DatabaseSchemaState.VERSION_0006.value,
    }
    backend_pid = event["backend_pid"]
    assert isinstance(backend_pid, int)
    return backend_pid
...
def _exercise_forced_termination_rollback_and_lock_release(
    ...
    *,
    ...
    assert owner_ready["transaction_state"] == DatabaseSchemaState.VERSION_0006.value
...
def _drop_post_0001_tables(connection: Connection) -> None:
    ...
    # Revision 0006 also adds a constraint to a v0 table, which dropping tables
    # cannot undo; leaving it behind would make the seeded database classify as
    # a v0 schema with an unexpected unique constraint rather than 0001.
