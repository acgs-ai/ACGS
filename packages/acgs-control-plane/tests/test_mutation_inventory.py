...
def test_mutation_inventory_preserves_canonical_and_legacy_truth(client: TestClient) -> None:
    ...
    inventory_paths = {
        (definition.method, definition.path) for definition in CANONICAL_MUTATION_DEFINITIONS
    }
    canonical_paths = {(contract.method, contract.path) for contract in canonical_contracts}
    assert len(CANONICAL_MUTATION_DEFINITIONS) == 4
    assert inventory_paths <= canonical_paths
    assert all(
...
def test_middleware_dispatch_replacement_is_refused_before_side_effect(
    ...
    middleware = client.app.user_middleware[0]

    async def replacement_dispatch(_request: Any, call_next: Any) -> Any:
        reached["dispatch"] = True
        return await call_next(_request)

    middleware.kwargs["dispatch"] = replacement_dispatch
    ...
    assert "replacement_dispatch" not in response.text
