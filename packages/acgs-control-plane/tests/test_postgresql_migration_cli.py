...
def test_public_function_hijack_cannot_mutate_during_status() -> None:
    ...
    assert payload["after"] == DatabaseSchemaState.VERSION_0010.value
...
def test_successful_upgrade_is_forward_only_and_idempotent() -> None:
    ...
    assert first_payload["after"] == DatabaseSchemaState.VERSION_0010.value

    second, second_payload = _invoke_cli("upgrade", acknowledge_forward_only=True)
    assert second.returncode == 0
    assert second_payload["before"] == DatabaseSchemaState.VERSION_0010.value
    assert second_payload["after"] == DatabaseSchemaState.VERSION_0010.value
    assert _version_rows() == [HEAD_REVISION]
