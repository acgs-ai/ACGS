...
_CURRENT_FORWARD_ONLY_REVISIONS = frozenset({"0001", "0002"})
...
def test_empty_and_existing_alpha_upgrade_head() -> None:
    ...
    assert empty_result.after.state is DatabaseSchemaState.VERSION_0002
    ...
    assert existing_result.after.state is DatabaseSchemaState.VERSION_0002
    assert _head_version(database_url) == HEAD_REVISION
...
def test_failed_migration_no_later_state(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert retried.after.state is DatabaseSchemaState.VERSION_0002
    assert _head_version(database_url) == HEAD_REVISION
    assert _rows(database_url, _LEGACY_TABLES) == legacy_rows
