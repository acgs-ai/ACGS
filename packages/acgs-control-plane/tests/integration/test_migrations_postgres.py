...
_CURRENT_FORWARD_ONLY_REVISIONS = frozenset({"0001", "0002", "0003", "0004", "0005", "0007"})
_CURRENT_REVERSIBLE_REVISIONS: frozenset[str] = frozenset({"0006", "0008"})
_LEGACY_ROW_COLUMNS = {
    "agents": (
        "id",
        "org_id",
...
def test_empty_and_existing_alpha_upgrade_head() -> None:
    ...
    assert empty_result.after.state is DatabaseSchemaState.VERSION_0008
    ...
    assert existing_result.after.state is DatabaseSchemaState.VERSION_0008
    assert _head_version(database_url) == HEAD_REVISION
...
def test_declared_reversible_round_trip() -> None:
    ...
    assert _CURRENT_REVERSIBLE_REVISIONS == {"0006", "0008"}
...
def test_failed_migration_no_later_state(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert retried.after.state is DatabaseSchemaState.VERSION_0008
    assert _head_version(database_url) == HEAD_REVISION
    assert _rows(database_url, _LEGACY_TABLES) == legacy_rows
